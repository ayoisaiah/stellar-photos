...
const photoCard = (photo) => {
  ...
  const photographerName = `${photo.user.first_name || ''} ${photo.user.last_name || ''}`;

  return `
      <li class="s-photo" id="photo-${photoId}" style="background: url(${backgroundPhoto}) rgb(239, 239, 239) top center no-repeat; background-size: cover;">
       <div class="s-photo-actions">
          <div class="top">
            <a class="user" href="${photographer}?utm_source=stellar-photos&utm_medium=referral&utm_campaign=api-credit" target="_blank" rel="noreferrer">
