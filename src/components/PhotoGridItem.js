...
class PhotoGridItem extends Component {
  ...
  render() {
    ...
    const backgroundStyle = {
      background: `url(${smallimageUrl}) #efefef center center no-repeat`
    };
    ...
    return (
      <Cell col={3}>
        <Card className="photo-grid__item" shadow={0} style={backgroundStyle}>
          <CardTitle expand />
          <CardActions style={{height: '52px', padding: '16px', background: 'rgba(0,0,0,0.2)', display: 'flex', boxSizing: 'border-box', alignItems: 'center', color: '#fff', justifyContent: 'space-between'}}>
